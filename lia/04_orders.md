...
Checkpoint 4 focuses on allowing users to buy products. Here are the
requirements:
...
- The shopping cart page lists products currently in the user's cart.
  For each product, its name, image, unit price (incl. discount if
  applicable), and total price (unit × quantity) are shown. The order's
  total is also displayed.
